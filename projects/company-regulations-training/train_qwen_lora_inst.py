"""
Qwen3 + LoRA 微调: 公司规章制度文本生成 (支持两阶段训练)
====================================================
用法:
  # 阶段1: 纯续写微调
  python train_qwen_lora_inst.py --data_dir ./data --output_dir ./output_stage1 --epochs 3 --max_length 512
  # 阶段2: 指令微调 (从阶段1续训, 方案推荐参数)
  python train_qwen_lora_inst.py --resume_from ./output_stage1/best_model \\
      --instruction_data ./data/instruction_train.json \\
      --instruction_val_data ./data/instruction_val.json \\
      --output_dir ./output_inst_v2 --epochs 3 --lr 5e-5 --max_length 768 --instruction_ratio 0.8 \\
      --early_stopping_patience 50 --min_delta 0.001 --overfit_gap_threshold 0.5
...
def main():
    parser = argparse.ArgumentParser(description="Qwen3-0.6B + LoRA 公司规章制度微调 (续写+指令)")
