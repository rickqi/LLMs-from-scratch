#!/usr/bin/env python3
"""
DPO 偏好对齐训练 — Qwen3 + LoRA (公司规章制度)
=============================================
将 DPO 损失直接作用于 LoRA 适配器，对齐制度 QA 输出质量。

用法:
  # 0.6B
  python train_dpo.py --base_model ./output_inst_v2/best_model --preference_data ./data/dpo_pairs.json

  # 1.7B (batch=1 防双模型 OOM, beta=0.05 更稳定)
  python train_dpo.py --model_name /home/models/ms_cache/Qwen/Qwen3-1___7B \\
      --base_model ./output_17b_inst/best_model \\
      --preference_data ./data/dpo_pairs.json \\
      --output_dir ./output_17b_dpo \\
      --batch_size 1 --beta 0.05 --epochs 1 --lr 5e-6

关键陷阱:
  - chosen/rejected 长度差 >50% → 模式坍塌, 必须过滤
  - 1.7B DPO 需 batch_size=1 (双模型 OOM)
  - beta=0.05 + 1 epoch 足矣
