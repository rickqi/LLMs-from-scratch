# 更高效的多头注意力（Multi-Head Attention）实现

- [mha-implementations.ipynb](mha-implementations.ipynb) 包含并比较了不同实现的多头注意力（multi-head attention）
...
&nbsp;
#### 前向和反向传播（Forward and backward pass）
...
&nbsp;
#### 编译后的前向和反向传播（Forward and backward pass after compilation）
